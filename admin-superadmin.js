...
const SA_PERMISSIONS = [
    ...
    { key:'logs',        label:'Admin Logs',      icon:'fa-list-check',      tab:12 },
...
async function saCheckIPStatus(ip) {
    ...
    try {
        ...
        const allowed = !(forceIP || saIPWhitelist.length > 0) || saIPWhitelist.includes(cleanIP);
        if (!cleanIP) {
            statusEl.textContent = '⚠️ IP konnte nicht ermittelt werden';
...
async function saSubmitReauth() {
    ...
    try {
        ...
        if ((forceIP || saIPWhitelist.length > 0) && !saIPWhitelist.includes(ip)) {
